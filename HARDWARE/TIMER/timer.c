...
extern void gizTimerMs(void);//机智云1ms计数函数，在gizwits_product.c里面定义
//定时器3中断服务程序	 
void TIM3_IRQHandler(void)
{ 		    		  			    
	if(TIM3->SR&0X0001)	//溢出中断
	{
		gizTimerMs();	//给机智云提供1ms时基			    				   				     	    	
	}				   
	TIM3->SR&=~(1<<0);	//清除中断标志位 	    
}
//通用定时器3中断初始化
...
void TIM3_Int_Init(u16 arr,u16 psc)
{
	...
	TIM3->CR1|=0x01;    //使能定时器3
  	MY_NVIC_Init(1,0,TIM3_IRQn,2);//抢占1，子优先级3，组2									 
}
//TIM3 PWM部分初始化 
//PWM输出初始化
//arr：自动重装值
...
{		 
	...
	MY_NVIC_Init(1,0,TIM5_IRQn,2);//抢占2，子优先级0，组2	   
}
