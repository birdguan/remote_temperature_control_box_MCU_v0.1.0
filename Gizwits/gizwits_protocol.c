...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagDeviceOnOff)
    {
        info->event[info->num] = EVENT_DeviceOnOff;
        info->num++;
        dataPoints->valueDeviceOnOff = gizDecompressionValue(DeviceOnOff_BYTEOFFSET,DeviceOnOff_BITOFFSET,DeviceOnOff_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flagTempSet)
    {
        info->event[info->num] = EVENT_TempSet;
        info->num++;
        dataPoints->valueTempSet = gizX2Y(TempSet_RATIO,  TempSet_ADDITION, issuedData->attrVals.valueTempSet); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueDeviceOnOff != cur->valueDeviceOnOff)
    {
        GIZWITS_LOG("valueDeviceOnOff Changed\n");
        ret = 1;
    }
    if(last->valueTempSet != cur->valueTempSet)
    {
        GIZWITS_LOG("valueTempSet Changed\n");
        ret = 1;
    }

    if(last->valueTempNow != cur->valueTempNow)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueTempNow Changed\n");
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizCompressValue(DeviceOnOff_BYTEOFFSET,DeviceOnOff_BITOFFSET,DeviceOnOff_LEN,(uint8_t *)devStatusPtr,dataPoints->valueDeviceOnOff);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));

    devStatusPtr->valueTempSet = gizY2X(TempSet_RATIO,  TempSet_ADDITION, dataPoints->valueTempSet); 
    devStatusPtr->valueTempNow = gizY2X(TempNow_RATIO,  TempNow_ADDITION, dataPoints->valueTempNow); 
